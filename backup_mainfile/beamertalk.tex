...
\usetheme{Goettingen} % This line sets the theme
...
\end{itemize}
\stepcounter{slidenum}
\note{ \arabic{slidenum} Hi class. How are you all doing? Recall that last class we coverd the
...
\begin{frame}[fragile]
	\stepcounter{slidenum}
\section{An Example} \subsection{Data Collection} \frametitle{Collect Data from our API}
	\begin{columns}
\column{0.5\textwidth}
...
\begin{frame}[fragile]
	...
	\note{Now if one were to examine the records in their entirety, one
		would discover that there's some negative values and some
		outliers. In particular there are outliers outside of 3
		standard deviations. 
		So we need to run some scripts to get rid of these. Then
	we can commence divying up the data into training and testing portions,
